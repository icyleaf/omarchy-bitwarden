...
- **Strict File & Socket Permissions & XDG Data Isolation**: Disk storage (`data.json`) resides outside the inotify-watched plugin directory at `$XDG_DATA_HOME/omawarden/data.json` (defaulting to `~/.local/share/omawarden/data.json` with automatic migration from legacy paths) to prevent hot-reload loops, and enforces `0600` owner-only permissions along with the daemon Unix socket (`omawarden.sock`) (see `docs/adr/0015-xdg-data-storage-location-and-migration.md`).
...
- **Config & Settings View**: Allows user configuration of `server_url`, `identity_url`, `download_dir`, `auto_lock_minutes`, `clipboard_clear_seconds`, `log_level`, `show_website_icons`, `check_updates`, and `remember_last_search`, with real-time CLI readiness indicators and an embedded Logs Viewer.
- **Search State Persistence & Reopen Interaction (`remember_last_search`)**: Optional user setting (default: `false`) that retains the active search query, category, vault/folder scopes, and list selection across overlay dismiss and reopen cycles while the vault remains unlocked. Upon reopening, focus is given to the search input with the cursor positioned at the end and text deselected, allowing immediate character-by-character backspace editing, seamless `Up`/`Down` list item navigation, and `Esc` query clearing. Under all circumstances, modal popups and revealed plaintext secrets are reset upon dismissal, and any vault lock event (manual `Ctrl+L`, idle timeout, screen lock, logout, server switch) unconditionally wipes the search state to uphold zero-leakage security invariants.
